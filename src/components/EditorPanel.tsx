...
import { AVAILABLE_IMAGES } from "@/lib/property";
...
export function EditorPanel({
  ...
  const [newRoomImage, setNewRoomImage] = useState(AVAILABLE_IMAGES[0].url);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <section>
          ...
          {activeRoom.hotspots.length === 0 ? (
            <p className="text-xs text-white/50 italic mb-2">
              No hotspots yet. Click anywhere on the panorama to add one.
            </p>
          ) : (
              ...
              {activeRoom.hotspots.map((h, i) => (
                ...
                >
                  ...
                  <div className="flex gap-1">
                    {[
                      { label: "↑", deg: 0, title: "Forward (away)" },
                      { label: "→", deg: 90, title: "Right" },
                      { label: "↓", deg: 180, title: "Back (toward you)" },
                      { label: "←", deg: -90, title: "Left" },
                    ].map((b) => (
                      <button
                        key={b.label}
                        type="button"
                        onClick={() =>
                          updateHotspot(activeRoom.id, i, {
                            arrowRotation: (b.deg * Math.PI) / 180,
                          })
                        }
                        title={b.title}
                        className="flex-1 rounded bg-white/5 px-1 py-0.5 text-xs text-white/70 hover:bg-white/15"
                      >
                        {b.label}
                      </button>
                    ))}
          ...
          <p className="text-[10px] text-white/40">
            Tip: while editor is open, click anywhere on the panorama to drop a
            new hotspot at that location.
          </p>
        </section>
        ...
        <section className="space-y-2">
          ...
          <button
            type="button"
            onClick={() => {
              if (confirm("Reset all changes to the default property?")) {
                reset();
              }
            }}
